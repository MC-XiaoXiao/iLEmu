...
struct RuntimeJitMemoryAggregate {
  ...
  static void subtract_current(
      JitPrecompileMemoryStats &target,
      const JitPrecompileMemoryStats &stats) noexcept {
    const auto subtract = [](std::size_t &value, std::size_t amount) noexcept {
  ...
  void retire(const void *runtime_key,
              std::optional<JitPrecompileMemoryStats> stats = std::nullopt)
      noexcept {
    const auto found = live_by_runtime.find(runtime_key);
    if (!stats && found != live_by_runtime.end()) stats = found->second;
...
[[nodiscard]] JitProfileMode parse_jit_profile_mode(
  ...
  if (configured) {
    if (*configured == "off") mode = JitProfileMode::Off;
    else if (*configured == "record-only") mode = JitProfileMode::RecordOnly;
    else if (*configured == "load-only") mode = JitProfileMode::LoadOnly;
    else if (*configured == "idle") mode = JitProfileMode::Idle;
    else if (*configured == "startup") mode = JitProfileMode::Startup;
    else {
      throw std::runtime_error{
          "--jit-profile-mode must be off, record-only, load-only, idle, or startup"};
    }
  }
...
void boot(const std::vector<std::string> &args, Output &output) {
  ...
  output.line(std::string{"[jit-profile] mode="} +
    ...
    if (!ran_thread) {
      ...
      const auto schedule_precompile_runtime =
        ...
        const auto next_phase =
            runtime->cpus->next_precompile_phase(target, source);
        if (!next_phase) {
  ...
  if (report_performance) {
    ...
    file_page_cache_bytes = static_cast<std::uint64_t>(
                                initial_runtime->memory->cached_file_page_count()) *
  ...
  output.line(
      ...
      std::to_string(artifact_compaction_telemetry
                         .temporary_cleanup_attempted.load(
                             std::memory_order_relaxed)) +
      " artifact-compaction-temporary-cleanup-succeeded=" +
      std::to_string(artifact_compaction_telemetry
                         .temporary_cleanup_succeeded.load(
                             std::memory_order_relaxed)) +
      " artifact-compaction-temporary-cleanup-failed=" +
      std::to_string(artifact_compaction_telemetry.temporary_cleanup_failed.load(
          std::memory_order_relaxed)) +
