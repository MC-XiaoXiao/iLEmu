...
[[nodiscard]] std::uint64_t duration_to_guest_ticks(std::uint64_t value,
...
void boot(const std::vector<std::string>& args, Output& output)
{
    ...
    const auto parse_startup_value =
        [](const std::optional<std::string>& value, std::string_view name,
    ...
    const auto startup_profile_budget_us =
        parse_startup_value(startup_profile_budget_us_value,
            "--jit-startup-profile-budget-us", 4'000U);
    ...
    const auto guest_processor_count =
        static_cast<std::size_t>(std::stoul(explicit_processor_count.value_or(
            std::to_string(default_processor_count))));
    ...
    const auto configured_jit_code_cache_size = jit_code_cache_size(args);
    ...
    if (display_mode == "sdl") {
        ...
        sdl_display =
            std::make_unique<SdlDisplay>(device.display, device.user_interface);
    ...
    const auto refresh_catalog_after_file_mutations = [&](bool
        ...
        catalog_refresh_task = host_resources.submit(
            ...
            [state = catalog_refresh_state,
                ...
                root = *rootfs, architecture = guest_architecture,
                base_revision, staged_manifest,
            ...
            }
            {
                ...
                child->kernel = std::make_unique<CompatibilityKernel>(
                    *child->memory, output, *rootfs, device,
                    activation_override, lockdown_profile);
            }
            if (inheritance ==
                CompatibilityKernel::ProcessInheritance::SpawnExec) {
                PerformanceLatencyScope latency {
    ...
    const auto wait_for_host_activity = [&](std::chrono::nanoseconds delay) {
        ...
        // until an unrelated SDL event wakes the loop; the next iteration will
        // drain the mailbox without changing Guest time or presentation
        // cadence.
        if (sdl_display && sdl_display->has_pending_presentation())
            return;
    ...
    while ((!bounded_execution || remaining_ticks != 0) &&
        ...
        if (live_control) {
            for (const auto& command : live_control->poll()) {
                ...
                case LiveControlCommandKind::Gesture:
                    ...
                    // Preserve stdin command order. In particular, "tap"
                ...
                case LiveControlCommandKind::Status: {
                    const auto submitted_frame =
                        initial_runtime->kernel->display_submitted_frames();
                    const auto frame = sdl_display
                                           ? sdl_display->presented_frames()
                                           : submitted_frame;
                    const auto active_process =
                        initial_runtime->kernel->active_client_process_id();
                    output.line(
        ...
        if (!ran_thread) {
            ...
            // input. Standalone SDL sessions block on SDL's event queue; GDB
