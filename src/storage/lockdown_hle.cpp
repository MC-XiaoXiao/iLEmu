...
bool is_lockdown_query(UserlandHleCall &call, std::string_view key_symbol,
  ...
  return call.objc_string_argument(2) == std::optional<std::string>{key_name};
}
...
void register_lockdown_hle(UserlandHleRegistry &registry,
  ...
  registry.register_function(
      ...
      [activated = *activated, profile](UserlandHleCall &call) {
        ...
        if (!is_lockdown_query(call, activation_state_key,
                               "ActivationState")) {
          // A process can query unrelated Lockdown keys before the activation
          // and brick state. Keep this boundary installed for later queries.
          call.resume_original_persistently();
          return;
        }
