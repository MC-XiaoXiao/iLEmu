...
namespace {
        ...
        std::uint32_t identifier, std::span<const std::byte> bytes)
    {
        ...
        const auto encoded_length =
            read_word(bytes, source_property_length_offset);
        ...
        const auto property_bytes =
            bytes.subspan(source_property_offset, encoded_length - 1U);
        if (!printable_ascii(property_bytes) ||
            bytes[source_property_offset + encoded_length - 1U] !=
                std::byte { }) {
            return std::nullopt;
        }
        const auto value_size_offset =
            (source_property_offset + encoded_length + 3U) &
            ~std::size_t { 3U };
        ...
        if (!std::isfinite(value))
            return std::nullopt;
        return SourceFloatProperty { read_word(bytes, source_identifier_offset),
