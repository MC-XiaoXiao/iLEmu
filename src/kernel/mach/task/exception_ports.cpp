...
constexpr std::uint32_t exception_reply_entry_size = 3U * sizeof(std::uint32_t);
...
task_object_for_name_locked(const KernelSharedState &state,
                            std::uint32_t caller, std::uint32_t name) {
  const auto object =
      resolve_name_with_right(state, caller, name, xnu792::ipc::Right::Send);
  if (!object || !state.task_port_pids.contains(*object))
    return std::nullopt;
  return object;
}

std::vector<ExceptionGroup>
group_exception_actions(const KernelSharedState::TaskExceptionActions &actions,
                        std::uint32_t mask) {
...
bool CompatibilityKernel::dispatch_mach_task_exception_message(
  ...
  if (request.identifier == set_id) {
    ...
    std::uint32_t port_object = 0;
    {
      ...
      if (!valid_wire || !exception_mask || !behavior || !flavor || !task ||
        ...
        for (std::size_t type = first_exception_type;
          ...
          if (action.port_object != port_object) {
            retain_kernel_send_right_locked(*shared_state_, port_object);
    ...
    registers[0] =
        write_simple_reply(memory_, request.address, request.local_port,
  ...
  const auto &arguments = xnu792::mig::task::task_get_exception_ports_arguments;
  const auto exception_mask =
      memory_.read32(request.address + arguments[1].request_offset);
  std::uint32_t task_object = 0;
  std::uint32_t result = mach_message_success;
  ...
  {
    std::lock_guard mach_lock{shared_state_->mach_mutex};
    const auto task = task_object_for_name_locked(*shared_state_, process_.pid,
                                                  request.remote_port);
    task_object = task.value_or(0);
    if (registers[2] < task_get_request_size || !exception_mask || !task ||
      ...
      const auto actions = shared_state_->task_exception_actions.find(*task);
  ...
  if (result != mach_message_success) {
    ...
    const auto reply_size =
        exception_reply_array_base_size + count * exception_reply_entry_size;
    ...
    for (std::size_t index = 0; index < maximum_exception_handlers; ++index) {
      reply.push_back(index < handler_names.size() ? handler_names[index]
  ...
  output_.write(
      "[mach] task_get_exception_ports caller=" + std::to_string(process_.pid) +
      " task=" + std::to_string(task_object) + " mask=0x" +
      hexadecimal(exception_mask.value_or(0)) +
