...
[[nodiscard]] std::optional<std::uint32_t> round_page_size(std::uint32_t size) {
  ...
  if (size == 0U || size > std::numeric_limits<std::uint32_t>::max() - mask) {
    return std::nullopt;
  }
...
find_remap_region(const AddressSpace &memory, std::uint32_t start,
                  std::uint32_t size, std::uint32_t mask) {
...
bool CompatibilityKernel::dispatch_mach_vm_remap_message(
  ...
  const auto descriptor_disposition =
      (*descriptor_metadata >> darwin::mig_wire::descriptor_disposition_shift) &
      0xffU;
  ...
  std::optional<std::uint32_t> source_pid;
  {
    ...
    source_pid =
        target_task_for_port(*shared_state_, process_.pid, *source_task_name);
  }
  ...
  if (result == kern_success && *source_pid == process_.pid) {
    ...
    if (region && region->address <= *source_address && region->end >= end) {
      if (auto pages = memory_.share_pages(*source_address, *size)) {
        source = SharedTaskMemoryRange{std::move(*pages), region->permissions};
      }
    }
  ...
  if (result == kern_success) {
    ...
    mapped = memory_.map_page_backings(
        *target_address, *size, source->permissions, pages,
  ...
  const auto protection = source ? darwin_permissions(source->permissions) : 0U;
