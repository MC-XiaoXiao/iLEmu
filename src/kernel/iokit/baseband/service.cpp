...
std::optional<ServiceProfile>
matching_service(std::span<const std::byte> matching) {
...
std::uint32_t ensure_service_locked(KernelSharedState &state,
  ...
  state.iokit_services.emplace(
      ...
      KernelSharedState::IOKitService{
          std::string{serial_multiplexer ? serial_multiplexer_class
                                        : service_class},
          {"IOService"},
          {},
          {},
...
std::optional<MethodResult> dispatch_connect_method(
  ...
  if (selector ==
    ...
    // retain the user-client ABI's strict shape instead of treating arbitrary
    // selectors as successful.
