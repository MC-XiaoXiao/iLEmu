...
// They are part of the accelerator user-client ABI, not an application rule.
...
// after the selector-4 reply.  A zero-sized IOConnectMapMemory request is an
// in/out request for this firmware-owned arena, so expose the smallest
// extent required by the ABI rather than a single guard page.
constexpr std::uint32_t minimum_shared_mapping_size = 0x4000U;
constexpr std::uint32_t shared_context_reply_size = 3U * sizeof(std::uint32_t);
...
string_property(std::string_view value) {
...
std::uint32_t write_map_reply(AddressSpace &memory, std::uint32_t address,
  ...
  const std::array<std::uint32_t, map_reply_size / sizeof(std::uint32_t)> reply{
...
bool request_targets_current_task_locked(const KernelSharedState &state,
  ...
  return owner != state.task_port_pids.end() && owner->second == process.pid;
}

std::optional<std::uint32_t>
rounded_mapping_size(std::uint32_t requested_size) {
  const auto size = std::max<std::uint32_t>(requested_size, AddressSpace::page_size);
...
std::optional<std::uint32_t> handle_map_memory_request(
  ...
  if (descriptor_count != 1U || task_name == 0 || !valid_memory_type ||
    ...
    return write_map_reply(memory, message_address, local_port, message_id,
                           mapped_size ? iokit_abi::bad_argument
                                       : iokit_abi::no_memory,
                           0, 0);
  }

  std::uint32_t mapped_address = 0;
  // IOConnectMapMemory's size argument is in/out.  The SGX shared arena is
  // requested with size zero and the user client returns its actual extent;
  // reporting the request back would make the firmware treat a valid mapping
  // as an empty arena.
  std::uint32_t exposed_size = *mapped_size;
  {
    ...
    if (existing != connection.memory_mappings.end() &&
        ...
        memory.mapped(existing->second.address, existing->second.mapped_size)) {
      ...
      if (!region ||
        ...
        return write_map_reply(memory, message_address, local_port, message_id,
                               iokit_abi::no_memory, 0, 0);
      }
      mapped_address = *region;
      connection.memory_mappings.emplace(
          memory_type,
...
std::optional<std::uint32_t> handle_unmap_memory_request(
  ...
  KernelSharedState::IOKitGraphicsConnectionState::MemoryMapping mapping;
  {
    ...
    auto connection = state.iokit_graphics_connections.find(connection_object);
    ...
    if (found == connection->second.memory_mappings.end() ||
        found->second.address != requested_address || requested_address == 0) {
      return write_simple_reply(memory, message_address, local_port, message_id,
                                iokit_abi::bad_argument);
    }
  ...
  return write_simple_reply(memory, message_address, local_port, message_id,
                            unmapped ? iokit_abi::success
                                     : iokit_abi::bad_argument);
}

std::optional<std::uint32_t> handle_add_client_request(
    AddressSpace &memory, KernelSharedState &state,
    const ProcessContext &process, std::uint32_t message_id,
    std::uint32_t message_address, std::uint32_t send_size,
    std::uint32_t receive_size, std::uint32_t connection_object,
    std::uint32_t local_port) {
  if (message_id != device_mig::id(device_mig::Routine::io_connect_add_client))
    return std::nullopt;
  ...
  const auto client_name =
      memory
          .read32(message_address +
                  device_mig::io_connect_add_client_arguments[1].request_offset)
          .value_or(0);
  {
    std::lock_guard lock{state.mach_mutex};
    if (!is_graphics_connection_locked(state, process, connection_object))
      return std::nullopt;
    // IOConnectAddClient names the shared connection with a task-local Mach
    // name. Resolve it before recording the relationship; the accelerator
    // does not expose a second service object for this handshake.
...
std::uint32_t ensure_service_locked(KernelSharedState &state,
  ...
  properties.emplace("IOGLESBundleName",
                     string_property(state.graphics_driver_bundle));
...
dispatch_connect_method(AddressSpace &memory, KernelSharedState &state,
  ...
  const auto shared_mapping_reply = [&]() -> std::optional<MethodResult> {
    const KernelSharedState::IOKitGraphicsConnectionState::MemoryMapping
        *mapping = nullptr;
    if (const auto local = connection.memory_mappings.find(shared_memory_type);
    ...
    for (std::size_t index = 0; index < words.size(); ++index) {
      ...
      for (std::size_t byte = 0; byte < sizeof(word); ++byte) {
        result[index * sizeof(word) + byte] = static_cast<std::byte>(
            (word >> (byte * 8U)) & 0xffU);
      }
    }
  ...
  case Selector::CreateShared:
    if (scalar_input.size() != 1U || scalar_input[0] != shared_memory_type ||
        !inband_input.empty() ||
        inband_output_capacity != 0U)
      return MethodResult{iokit_abi::bad_argument, {}, {}};
...
std::optional<std::uint32_t> handle_mach_request(
    AddressSpace &memory, KernelSharedState &state,
    const ProcessContext &process, std::uint32_t message_id,
    std::uint32_t message_address, std::uint32_t send_size,
    std::uint32_t receive_size, std::uint32_t connection_object,
  ...
  if (const auto result = handle_map_memory_request(
          memory, state, process, message_id, message_address,
          send_size, receive_size, connection_object, local_port)) {
    return result;
  }
  if (const auto result = handle_unmap_memory_request(
          memory, state, process, message_id, message_address,
          send_size, receive_size, connection_object, local_port)) {
    return result;
  }
  return handle_add_client_request(
      memory, state, process, message_id, message_address, send_size,
...
void close_connection(AddressSpace &memory, KernelSharedState &state,
      ...
      mappings;
  {
    ...
    for (const auto &[memory_type, mapping] : connection->second.memory_mappings) {
