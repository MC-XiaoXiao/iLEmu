...
KernelSharedState::IOKitRegistryProperty number_property(std::uint32_t value) {
...
KernelSharedState::IOKitRegistryProperty
array_property(std::vector<KernelSharedState::IOKitRegistryProperty> value) {
...
KernelSharedState::IOKitRegistryProperty dictionary_property(
  ...
  KernelSharedState::IOKitRegistryProperty property;
  property.kind = KernelSharedState::IOKitRegistryProperty::Kind::Dictionary;
...
streams_property(const IOKitAudioAbiProfile &profile,
  ...
  for (const auto &stream : device.streams) {
    if (stream.direction != direction)
      continue;
    std::map<std::string, KernelSharedState::IOKitRegistryProperty> properties;
    properties.emplace(profile.registry.stream_id,
                       number_property(stream.identifier));
...
std::optional<std::uint32_t> handle_notification_port_request(
  ...
  output.write(
      "[iokit-audio] notification-port pid=" + std::to_string(process.pid) +
...
std::optional<std::uint32_t> handle_map_memory_request(
  ...
  const auto exposed_size =
      engine_status ? profile.memory.engine_status_size : stream->buffer_size;
                           ...
                           ~(AddressSpace::page_size - 1U);
  {
    ...
    if (existing != connection.memory_mappings.end() &&
      ...
      connection.memory_mappings[memory_type] =
          KernelSharedState::IOKitAudioConnectionState::MemoryMapping{
...
std::optional<std::uint32_t> handle_unmap_memory_request(
  ...
  std::uint32_t mapped_size = 0;
  {
    ...
    if (descriptor_count != 1U || task_name == 0 || !known_memory_type ||
        !request_targets_current_task_locked(state, process, task_name) ||
        connection == state.iokit_audio_connections.end() ||
        mapping.address != requested_address || requested_address == 0) {
      return write_simple_reply(memory, message_address, local_port, message_id,
                                iokit_abi::bad_argument);
    }
...
dispatch_connect_method(KernelSharedState &state, const ProcessContext &process,
  ...
  if (selector == profile.selectors.set_nominal_sample_rate) {
    ...
    connection.nominal_sample_rate.assign(inband_input.begin(),
                                          inband_input.end());
    return MethodResult{iokit_abi::success, {}};
  }
...
handle_mach_request(AddressSpace &memory, Output &output,
  ...
  if (const auto result = handle_notification_port_request(
          memory, output, state, process, message_id, message_address,
  ...
  return handle_unmap_memory_request(memory, output, state, process, message_id,
