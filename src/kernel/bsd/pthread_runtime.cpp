...
bool DarwinPthreadRuntime::enqueue_workitem(DarwinWorkqueueItem item,
                                            bool front) {
...
bool DarwinPthreadRuntime::remove_workitem(std::uint32_t address,
...
bool DarwinPthreadRuntime::should_create_worker(
  ...
  if (overcommit)
    return true;
  const auto running = std::count_if(
      workers_.begin(), workers_.end(), [](const auto &entry) {
        return !entry.second.idle;
      });
...
std::optional<DarwinWorkqueueWorker>
DarwinPthreadRuntime::worker(std::uint32_t processor) const {
...
std::optional<DarwinWorkqueueWorker>
DarwinPthreadRuntime::idle_worker() const {
...
void DarwinPthreadRuntime::mark_worker_running(std::uint32_t processor,
                                               std::uint32_t priority) {
...
bool DarwinPthreadRuntime::set_target_concurrency(std::uint32_t priority,
...
bool CompatibilityKernel::service_bsd_workqueue(Cpu &cpu) {
  ...
  if (idle_worker) {
    ...
    const auto cpsr = pthread_start_cpsr(
        registration->workqueue_thread_start);
    ...
    const auto updated =
        ...
        thread_state_update_handler_(process_.pid, idle_worker->processor,
                                     guest_state) &&
        (!thread_pointer_update_handler_ ||
         thread_pointer_update_handler_(process_.pid, idle_worker->processor,
                                        idle_worker->pthread_address));
    ...
    pthread_runtime_.mark_worker_running(idle_worker->processor,
                                         next_item->priority);
    output_.write("[pthread] workqueue reuse pid=" +
                  std::to_string(process_.pid) + " slot=" +
                  std::to_string(idle_worker->processor) + " item=" +
                  std::to_string(next_item->address) + " priority=" +
                  std::to_string(next_item->priority) + "\n");
  ...
  auto state = workqueue_thread_state(*registration, worker, *next_item,
                                      false);
  ...
  if (!thread_state_update_handler_ ||
      !thread_state_update_handler_(process_.pid, worker.processor,
                                    guest_state) ||
      (thread_pointer_update_handler_ &&
       !thread_pointer_update_handler_(process_.pid, worker.processor,
                                       worker.pthread_address)) ||
  ...
  output_.write("[pthread] workqueue create pid=" +
                std::to_string(process_.pid) + " slot=" +
                std::to_string(worker.processor) + " item=" +
                std::to_string(next_item->address) + " pthread=" +
                std::to_string(worker.pthread_address) + " priority=" +
                std::to_string(next_item->priority) + "\n");
...
bool CompatibilityKernel::dispatch_bsd_pthread(Cpu &cpu,
                                                std::uint32_t number) {
  const auto profile =
  ...
  case 360: { // bsdthread_create, Darwin 10 ARM32 v1
    ...
    if (!created) {
      if (allocation)
        static_cast<void>(memory_.unmap(allocation->first,
                                        allocation->second));
    ...
    if (!thread_state_update_handler_ ||
        !thread_state_update_handler_(
            process_.pid, static_cast<std::uint32_t>(created->processor),
            darwin::arm_thread::GeneralState{
                state[0], state[1], state[2], state[3], state[4], state[5],
                state[6], state[7], state[8], state[9], state[10], state[11],
                state[12], state[13], state[14], state[15], cpsr}) ||
        (thread_pointer_update_handler_ &&
         !thread_pointer_update_handler_(
             process_.pid, static_cast<std::uint32_t>(created->processor),
             pthread_address))) {
    ...
    output_.write("[pthread] create pid=" +
                  std::to_string(process_.pid) + " slot=" +
                  std::to_string(created->processor) + " start=" +
                  std::to_string(registration->thread_start) + " function=" +
                  std::to_string(user_function) + " pthread=" +
                  std::to_string(pthread_address) + "\n");
  ...
  case 361: { // bsdthread_terminate, Darwin 10 ARM32 v1
    ...
    std::uint32_t semaphore_result = darwin::mach::success;
    {
      ...
      if (thread_object) {
        ...
        mach_support::terminate_receive_object_locked(*shared_state_,
                                                      *thread_object);
      }
      ...
      if (thread_port != xnu792::ipc::null_name &&
        ...
        const auto entry =
            shared_state_->mach_namespaces.lookup(process_.pid, thread_port);
        if (entry) {
          const auto has = [&](xnu792::ipc::Right right) {
            return (entry->type & xnu792::ipc::type_mask(right)) != 0;
          };
          const auto right =
              has(xnu792::ipc::Right::Send)
          ...
          static_cast<void>(mach_support::modify_port_references_locked(
              *shared_state_, process_.pid, thread_port, right, -1));
        }
      }
    }
    wake_thread_and_maybe_preempt(cpu, woken_thread);
    output_.write("[pthread] terminate pid=" +
                  std::to_string(process_.pid) + " slot=" +
                  std::to_string(processor) + " stack=" +
                  std::to_string(stack_address) + " size=" +
  ...
  case 366: { // bsdthread_register, Darwin 10 ARM32 v1
    ...
    output_.write("[pthread] register pid=" +
                  std::to_string(process_.pid) +
                  " pthread-size=" +
                  std::to_string(registration.pthread_size) +
  ...
  case 368: { // workq_kernreturn, Darwin 10 ARM32 v1
    ...
    const auto priority = raw_priority & ~DarwinPthreadRuntime::workqueue_overcommit;
    if (operation == queue_add) {
      ...
      if (priority >= DarwinPthreadRuntime::workqueue_priority_count ||
        ...
        bsd_error(cpu, priority >=
                               DarwinPthreadRuntime::workqueue_priority_count
                           ? bsd_support::invalid_argument
                           : darwin::error::no_memory);
        return true;
      }
    ...
    if (operation == queue_remove) {
      if (priority >= DarwinPthreadRuntime::workqueue_priority_count) {
        bsd_error(cpu, bsd_support::invalid_argument);
      } else if (!pthread_runtime_.remove_workitem(item_address,
                                                   priority)) {
    ...
    if (operation == thread_return) {
      ...
      if (next_item) {
        ...
        pthread_runtime_.mark_worker_running(processor,
                                             next_item->priority);
        output_.write("[pthread] workqueue continue pid=" +
                      std::to_string(process_.pid) + " slot=" +
                      std::to_string(processor) + " item=" +
                      std::to_string(next_item->address) + " priority=" +
                      std::to_string(next_item->priority) + "\n");
        return true;
      }
      ...
      output_.write("[pthread] workqueue park pid=" +
                    std::to_string(process_.pid) + " slot=" +
                    std::to_string(processor) + "\n");
