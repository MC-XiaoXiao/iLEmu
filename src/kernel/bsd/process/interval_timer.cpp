...
std::optional<std::uint64_t> read_time_value(const AddressSpace &memory,
  ...
  const auto raw_microseconds = memory.read32(address + microseconds_offset);
  ...
  if (seconds < 0 || microseconds < 0 ||
      microseconds >= static_cast<std::int32_t>(microseconds_per_second)) {
    return std::nullopt;
  }
...
std::optional<TimerValue> read_timer_value(const AddressSpace &memory,
                                           std::uint32_t address) {
  if (address == 0U || address > std::numeric_limits<std::uint32_t>::max() -
                                     timer_value_size + 1U) {
    return std::nullopt;
  }
  const auto interval = read_time_value(memory, address + interval_offset);
...
bool write_time_value(AddressSpace &memory, std::uint32_t address,
...
bool write_timer_value(AddressSpace &memory, std::uint32_t address,
                       const TimerValue &timer) {
  if (address == 0U || address > std::numeric_limits<std::uint32_t>::max() -
                                     timer_value_size + 1U) {
    return false;
  }
  return write_time_value(memory, address + interval_offset, timer.interval) &&
         write_time_value(memory, address + value_offset, timer.value);
}

TimerValue current_value(const KernelSharedState::ProcessIntervalTimer &timer,
                         std::uint64_t now) {
  return TimerValue{timer.interval, timer.deadline && *timer.deadline > now
                                        ? *timer.deadline - now
                                        : 0U};
}

std::optional<std::uint64_t> deadline_after(std::uint64_t now,
...
bool dispatch(Cpu &cpu, AddressSpace &memory, Output &output,
  ...
  }
  {
    ...
    if (registers[2] != 0U &&
        !write_timer_value(memory, registers[2], current_value(timer, now))) {
      set_error(cpu, darwin::error::bad_address);
      return true;
    }
...
std::optional<std::uint64_t> next_deadline(KernelSharedState &state,
                                           std::uint32_t process_id) {
  std::lock_guard lock{state.mach_mutex};
  const auto timer = state.process_interval_timers.find(process_id);
  return timer == state.process_interval_timers.end() ? std::nullopt
...
bool service_due(KernelSharedState &state, std::uint32_t process_id,
  ...
  std::lock_guard lock{state.mach_mutex};
  const auto timer = state.process_interval_timers.find(process_id);
  if (timer == state.process_interval_timers.end() || !timer->second.deadline ||
