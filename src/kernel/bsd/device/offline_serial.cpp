...
void State::set_attributes(
    const darwin::tty::Arm32Attributes &attributes) {
  const std::lock_guard lock{mutex_};
...
void State::consume_h4_byte(std::byte value) {
  ...
  const auto csr_bccmd =
      command_[1] == std::byte{0x00} && command_[2] == std::byte{0xfc} &&
      parameter_length >= 11;
  if (csr_bccmd) {
    // CSR BCCMD replies use a vendor event rather than Command Complete.
    // Mirror the request envelope, turn REQ into RESP, and report an unknown
    // variable. The guest retains ownership of policy and exits its command
    // path immediately instead of waiting for a controller that is absent.
    ...
    receive_queue_.insert(receive_queue_.end(), response.begin(),
                          response.end());
  } else {
    // H4 Command Complete: one command packet may continue, original opcode,
    // and a standard HCI Hardware Failure status.
    const std::array response{
        h4_event_packet, command_complete_event, std::byte{0x04},
        std::byte{0x01}, command_[1], command_[2], hardware_failure_status};
    receive_queue_.insert(receive_queue_.end(), response.begin(),
                          response.end());
  }
  command_.clear();
