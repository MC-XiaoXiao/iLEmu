...
bool CompatibilityKernel::send_host_socket_bytes(
    Cpu &cpu, std::uint32_t fd, std::vector<std::byte> bytes,
    std::vector<std::byte> destination, bool nonblocking) {
  ...
  if (result.status == HostSocketStatus::WouldBlock) {
    ...
    pending_host_writes_.insert_or_assign(
        processor,
        PendingHostWrite{fd, host->second, std::move(bytes),
                         std::move(destination)});
    process_.waiting_for_events = true;
    bsd_success(cpu, 0);
    output_.write("[network] write wait pid=" +
...
bool CompatibilityKernel::send_socket_message(
  ...
  if ((*iov_count != 0 &&
      ...
      (*name_length != 0 &&
       (*name_address == 0 ||
        !memory_.accessible(*name_address, *name_length,
                            MemoryPermission::Read))) ||
      (*control_length != 0 &&
       (*control_address == 0 ||
  ...
  if (host != host_sockets_.end()) {
    ...
    return send_host_socket_bytes(cpu, fd, std::move(payload),
