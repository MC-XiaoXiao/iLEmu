...
public:
  ...
  [[nodiscard]] bool enqueue_workitem(DarwinWorkqueueItem item,
                                      bool front = false);
  [[nodiscard]] std::optional<DarwinWorkqueueItem> take_workitem();
  [[nodiscard]] bool remove_workitem(std::uint32_t address,
                                     std::uint32_t priority);
  [[nodiscard]] bool should_create_worker(std::uint32_t priority,
                                          bool overcommit) const noexcept;
  [[nodiscard]] bool add_worker(DarwinWorkqueueWorker worker);
  [[nodiscard]] std::optional<DarwinWorkqueueWorker>
  worker(std::uint32_t processor) const;
  ...
  [[nodiscard]] bool set_target_concurrency(std::uint32_t priority,
  ...
  void inherit_from(const DarwinPthreadRuntime &parent, bool fork) noexcept {
    ...
    // XNU constructs a fresh workqueue for a child process.  The pthread ABI
    // registration survives fork, but kernel workqueue state does not.
    reset_workqueue();
  }
