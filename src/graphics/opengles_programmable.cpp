...
std::optional<std::vector<float>> read_floats(UserlandHleCall &call,
...
void set_previous_times_texture_alpha(GlesTextureEnvironment &environment,
                                      bool inverted) {
...
void set_interpolation(GlesTextureEnvironment &environment,
    ...
    environment.rgb_sources[0] = previous_is_first
                                     ? gles_abi::previous
                                     : gles_abi::texture_source;
    environment.rgb_sources[1] = previous_is_first
                                     ? gles_abi::texture_source
                                     : gles_abi::previous;
...
OpenGlesHle::programmable_draw_state(const ContextState &context) const {
    ...
    const auto array_for = [&](std::string_view semantic)
        ...
        return array == context.generic_arrays.end() ? nullptr
                                                     : &array->second;
    };
    ...
    for (std::size_t unit = 0; unit < result.texture_arrays.size(); ++unit) {
        ...
        if (const auto *scale = programs_.uniform(
            ...
            result.texture_scales[unit] = {scale->values[0], scale->values[1]};
        }
    }
    ...
    if (expression.find("mix (s1, s0, function_arg)") !=
        ...
        set_interpolation(
            unit1, true,
            scalar_uniform(programs_, context.current_program, "function_arg",
                           0.0F));
    ...
    } else if (expression.find("gl_FragColor = s0") !=
               std::string_view::npos) {
        unit0.mode = gles_abi::replace;
    }
    return result;
...
void OpenGlesHle::register_programmable_gles(
    ...
    add("_glCompileShader", [this](UserlandHleCall &call) {
        ...
        shader->compiled = !shader->source.empty() &&
                           shader->source.find("void main") !=
                               std::string::npos;
    });
    ...
    add("_glBindAttribLocation", [this](UserlandHleCall &call) {
        ...
        if (!name || call.argument(1) >=
                         gles_abi::maximum_vertex_attributes ||
            !programs_.bind_attribute(call.argument(0), call.argument(1),
                                      *name)) {
            set_gl_error(call, gles_abi::invalid_value);
        }
    });
    ...
    add("_glGetAttribLocation", [this](UserlandHleCall &call) {
        const auto name = call.string_argument(1, 256U);
        const auto location = name
                                  ? programs_.attribute(call.argument(0), *name)
                                  : std::nullopt;
        call.set_return(location ? *location
    ...
    add("_glVertexAttribPointer", [this](UserlandHleCall &call) {
        ...
        const auto valid_type = type == gles_abi::byte ||
                                type == gles_abi::unsigned_byte ||
                                type == gles_abi::short_type ||
                                type == gles_abi::unsigned_short ||
                                type == gles_abi::fixed ||
                                type == gles_abi::float_type;
        if (context == nullptr) {
            set_gl_error(call, gles_abi::invalid_operation);
        } else if (index >= gles_abi::maximum_vertex_attributes || size < 1 ||
                   size > 4 || stride < 0 || !valid_type) {
            set_gl_error(call, !valid_type ? gles_abi::invalid_enum
                                           : gles_abi::invalid_value);
        } else {
            const auto enabled = context->generic_arrays[index].enabled;
            context->generic_arrays[index] = ContextState::ArrayPointer{
                static_cast<std::uint32_t>(size), type,
                static_cast<std::uint32_t>(stride), call.argument(5),
    ...
    const auto set_float_uniform = [this](UserlandHleCall &call,
        ...
        if (!values || !programs_.set_uniform(context->current_program,
                                               location, *values)) {
            set_gl_error(call, gles_abi::invalid_operation);
        }
    };
    ...
    add("_glUniform1i", [this](UserlandHleCall &call) {
        ...
        if (location == -1)
            return;
        if (context == nullptr || context->current_program == 0U ||
            !programs_.set_uniform(
                context->current_program, location,
    ...
    const auto get_info_log = [this](UserlandHleCall &call) {
        const auto exists = call.symbol() == "_glGetShaderInfoLog"
                                ? programs_.shader(call.argument(0)) != nullptr
        ...
        if (!write_empty_log(call, call.argument(1), call.argument(2),
                             call.argument(3))) {
            set_gl_error(call, gles_abi::invalid_value);
        }
    };
