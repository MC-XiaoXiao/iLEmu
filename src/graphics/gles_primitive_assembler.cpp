...
bool is_triangle_mode(std::uint32_t mode) {
    return mode == gles_abi::triangles || mode == gles_abi::triangle_strip ||
...
bool append_line_segment(std::vector<GlesRasterVertex>& destination,
                         const GlesRasterVertex& start,
                         const GlesRasterVertex& end,
                         const GlesRasterState& state) {
    const auto start_w = start.position[3];
    const auto end_w = end.position[3];
    if (!std::isfinite(start_w) || !std::isfinite(end_w) || start_w == 0.0F ||
        end_w == 0.0F) {
        return false;
    }
...
std::optional<GlesPrimitiveBatch>
GlesPrimitiveAssembler::assemble(std::span<const GlesRasterVertex> vertices,
                                 std::uint32_t mode,
                                 const GlesRasterState& state) {
    if (!supports(mode))
        return std::nullopt;
    ...
    const auto append = [&](std::size_t start, std::size_t end) {
        return append_line_segment(expanded, vertices[start], vertices[end],
                                   state);
    };
    if (mode == gles_abi::lines) {
