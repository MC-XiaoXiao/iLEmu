...
class GlesProgramState {
  public:
    ...
    [[nodiscard]] bool attach_shader(std::uint32_t program,
                                     std::uint32_t shader);
    [[nodiscard]] bool bind_attribute(std::uint32_t program,
                                      std::uint32_t index,
                                      std::string name);
    [[nodiscard]] bool link(std::uint32_t program);
    [[nodiscard]] std::optional<std::uint32_t>
    attribute(std::uint32_t program, std::string_view name) const;
    [[nodiscard]] std::int32_t uniform_location(std::uint32_t program,
                                                std::string_view name);
    [[nodiscard]] bool set_uniform(std::uint32_t program,
                                   std::int32_t location,
                                   std::span<const float> values);
    [[nodiscard]] bool set_uniform(std::uint32_t program,
                                   std::int32_t location,
                                   std::int32_t value);
    [[nodiscard]] const Uniform *uniform(std::uint32_t program,
                                         std::string_view name) const;
    [[nodiscard]] std::string_view shader_source(std::uint32_t program,
